...
|--------|------|------|
| 0 | 成功 | 命令执行成功 |
| < 0 | 出错 | fork/pipe/open 失败 |
| 出错号 | 区分错误类型 | -1=系统调用失败, -2=语法错误, -3=命令不存在 |

**核心原则**：谁离错误最近谁处理，处理不了的往上传。
...
int create_pipe(int fds[2]) {
    if (pipe(fds) == -1) {
        perror("pipe");
        return -1;    // 返回错误码，让调用者决定怎么办
    }
    return 0;
}

// 中层函数：收到错误，判断严重程度
int setup_command(char *cmd) {
    int fds[2];
    if (create_pipe(fds) == -1) {
        // pipe 失败是致命错误，整个命令没法执行
        return -1;    // 继续往上抛
    }
    return 0;
}
...
// 最上层：兜底，做最后决策
int main() {
    if (setup_command("ls -l") == -1) {
        fprintf(stderr, "[FATAL] 管道创建失败，shell 退出\n");
        return 1;
    }
}
```

**不要做**：在底层 `exit(1)` 一把梭——底层一 exit 外面全挂了，别人连收尸的机会都没有。
...
| 宏 | 含义 | 什么时候用 |
|----|------|-----------|
| LOG_ERROR | 错误但还能继续 | fork 某个命令失败，跳过去执行下一个 |
| LOG_FATAL | 致命错误必须死 | pipe 失败，shell 没法工作 |
| LOG_INFO | 正常运行信息 | 命令执行完成 |
| LOG_DEBUG | 调试信息 | pipe fd 号、信号到达 |

对比：
```
// 之前：printf("[子进程] 读到 %zd 字节: %s", n, buf);
// 之后：LOG_DEBUG("子进程从 pipe 读到 %zd 字节", n);
```

出 bug 时只需把 DEBUG 宏打开，就能看到完整的函数名和文件行号。不需要手动在所有 printf 里加函数名。

### 三、调试：三个必杀技（优先级从高到低）

| 优先级 | 招数 | 命令 | 场景 |
|--------|------|------|------|
| 1 | strace | `strace -f -e trace=process,file ./my_shell` | shell 不执行命令，看是不是 fork/exec 没调 |
| 2 | GDB | `gdb --args ./my_shell` → `break main` → `run` → `bt` | segment fault，看调用栈找哪行崩 |
| 3 | 二分注释法 | 注释掉一半代码，看 bug 还在不在，逐步缩小范围 | 不知道哪里出问题时的核武器 |
| 最后 | printf 大法 | 加 printf 重新编译跑 | 便捷但不应该成为唯一手段 |

**二分法定位是所有 bug 排查的通用模式**——把代码切成两半，测哪一半有 bug，再切再测，直到定位到一行代码。
...
# 重定向
echo hello > /tmp/test.txt && cat /tmp/test.txt    # 输出重定向
cat < /tmp/test.txt | grep hello                    # 输入重定向

# 边界条件
(空回车)                     # 空命令不崩
   (只有空格)               # 纯空格不崩
nosuchcmd                    # 不存在的命令，应打印报错不闪退
ls |                         # 管道后面没接命令，应报错不崩
```

每条都要跑，每条都有预期结果。**写代码前先写好测试用例，开发时跑通了才叫做完。**
...
```
parse_cmd()   — 只负责解析命令，不负责执行
exec_cmd()    — 只负责执行，不负责管道连接
setup_pipe()  — 只负责 fd 管理
```

600 行的 shell 还全塞在 main 里，两周后回来看自己的代码会骂街。画模块图 → 每个模块一个函数 → 每个函数一个入口一个出口。
